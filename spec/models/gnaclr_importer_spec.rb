...
describe GnaclrImporter, 'create' do
  ...
  let(:subject) { Factory(:gnaclr_importer, :master_tree => master_tree, :url => 'http://example.com') }
  it 'sets the url' do
...
describe GnaclrImporter, 'import a tree for a valid dwc archive' do
  let(:master_tree) { Factory(:master_tree) }
  let(:subject) { Factory(:gnaclr_importer,
                    :master_tree    => master_tree,
                    :url            => "file:///#{Rails.root.join('features', 'support', 'fixtures', 'cyphophthalmi.tar.gz')}") }

  it "should import darwin core file into a reference tree" do
    subject.reference_tree.should be_nil
    subject.import
    reference_tree = subject.reference_tree
    reference_tree.is_a?(ReferenceTree).should be_true
    reference_tree.nodes.size.should > 100
  end

  it "should not import the same core file twice" do
    subject.reference_tree.should be_nil
    subject.import
    reference_tree1 = subject.reference_tree
    reference_tree1.is_a?(ReferenceTree).should be_true
    importer2 = Factory(:gnaclr_importer,
                    :master_tree    => master_tree,
                    :revision       => reference_tree1.revision,
                    :url            => "file:///#{Rails.root.join('features', 'support', 'fixtures', 'cyphophthalmi.tar.gz')}")
    importer2.import
    importer2.reference_tree.should == reference_tree1
    importer3 = Factory(:gnaclr_importer,
                    :master_tree    => master_tree,
                    :url            => "file:///#{Rails.root.join('features', 'support', 'fixtures', 'cyphophthalmi.tar.gz')}")
    importer3.import
    importer3.reference_tree.should_not == reference_tree1
  end
end
#   before do
...
#   let(:master_tree) { Factory(:master_tree, :state => 'importing') }
#   subject { GnaclrImporter.create(:url            => "file:///#{Rails.root.join('features', 'support', 'fixtures', 'cyphophthalmi.tar.gz')}",
#                                :master_tree => master_tree) }
#
#   before do
