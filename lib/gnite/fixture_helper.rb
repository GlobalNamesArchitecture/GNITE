...
module Gnite::FixtureHelper
  ...
  def self.get_master_tree2(options = {})
    ...
    bulk_add6 = Factory(:action_bulk_add_node, :tree_id => tree.id, :parent_id => genera_ids[5], :json_message => '{"do": ["Crocodilosa kittenbergeri Caporiacco, 1947", "Crocodilosa leucostigma (Simon, 1885)", "Crocodilosa maindroni (Simon, 1897)", "Crocodilosa ovicula (Thorell, 1895)", "Crocodilosa virulenta (O. P.-Cambridge, 1876)"]}')
    ActionBulkAddNode.perform(bulk_add2.id)
